...
heroImage: "blog/ai-chatgpt-gemini"
...
import CollapsibleQuote from "@src/components/CollapsibleQuote";
...
This topic was not something I would have thought about and I am glad this class introduced me to this topic.
...
<CollapsibleQuote label="ChatGPT Response" client:load>
...
<CollapsibleQuote label="Gemini Response" client:load>
...
The prevalence of these particular male figures is understandable given their outsized historical impact: (1) Newton revolutionized physics and mathematics with his laws of motion and calculus. (2) Einstein transformed our understanding of space, time, and the universe with his theory of relativity. (3) Gandhi pioneered non-violent resistance that influenced civil rights movements worldwide and (4) Lincoln preserved the American Union while abolishing slavery4. Their inclusion reflects both their genuine achievements and their extensive documentation in historical records.
...
In analyzing both ChatGPT's and Gemini's responses to feedback about bias, several key findings emerged.
Both models demonstrated notable self-awareness by acknowledging their initial biases in selecting historical figures.

Gemini explicitly recognized its male bias and took concrete steps to address this by providing a list that included only women who were deemed influential in shaping human civilianization.
This is interesting, however, because if you take that response in a vacuum, you still have a very biased response.
Responding to the question with only women figures bypasses the challenge of still getting an unbiased response.

On the other hand, ChatGPT adjusted its approach by including a more diverse range of influential people across gender, culture, and time periods.
It is interesting to note, however, that ChatGPT did expand its number of examples to help include women.
I am not sure if this is better, but it does give a more holistic unbiased response.

During these adjustments, both models maintained their high content quality standards while improving representation.
Each person of historical influence inclusion came with substantive justifications focused on their achievements and influence, not their demographic traits, however again in Gemini’s case, it did limit the responses to just women.

To address these biases in AI systems, several key mitigation strategies should be implemented.
First, systematic bias checks must be incorporated into AI development and testing phases.
Additionally, it's crucial to ensure diverse perspectives are included in AI training data curation.
Organizations should develop and maintain clear protocols for identifying and addressing biases in AI outputs.
Transparency about potential biases and system limitations should be maintained throughout the development and deployment process.
Finally, users should be actively encouraged to question the system's outputs and provide feedback on potential biases they observe.

The findings from this analysis emphasize that AI systems require ongoing monitoring and refinement to ensure equitable service to all users while upholding high quality standards.
Regular oversight is crucial, as these systems can potentially reinforce societal biases if not properly managed.
This has been demonstrated through our examination of ChatGPT and Gemini's responses to gender bias in historical figure selection.
